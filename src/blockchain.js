...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try{
                ...
                if(current_height !== -1){
                    block.previous_block_hash = self.chain[current_height].hash;
                };
                ...
                //Validate chain
                let error_log = await self.validateChain()
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if((current_time - request_time) < 500*60 && bitcoinMessage.verify(message, address, signature)){
                let block = new BlockClass.Block({star : star, owner: address});
                resolve(self._addBlock(block));
            }else{
                reject("Can't submit Star");
            };
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for(let i = 0; i <= self.height; i++){
                let block = self.chain[i]
                let block_valid = await block.validate()
                let hash_valid = true;
                if(block.height !== 0){
                    hash_valid = block.previous_block_hash == self.chain[i-1].hash
                }
                
                if(!block_valid || !hash_valid){
                    let error = {
                        height: i,
                        block_valid,
                        hash_valid,
